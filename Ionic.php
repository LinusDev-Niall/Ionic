...
if ($mode == 'string') {
    ...
    if (!isset($filePath) ||
        (isset($filePath) && ((strlen($filePath) < $sec_fp_min_length || strlen($filePath) > $sec_fp_max_length)))
    ) {
    ...
    foreach ($docs as $item) {
        ...
        if ($sim > $ion_sim_threshold) {
            $result = $coll->insertOne(
                array('md5' => md5($left), 'text' => base64_encode($left), 'sim' => $sim, 'created' => time()));
...
if ($mode == 'file' && isset($file[2])) {
    ...
    $data = isset($newDocId) ? statusSuccessInsert($msg_valid_ioncube, $newDocId) : statusSuccess($msg_valid_ioncube);
} else {
    $data = isset($newDocId) ? statusSuccessInsert($msg_abrupt_f_end, $newDocId) : statusSuccess($msg_abrupt_f_end);
}
die($data);
